...
void AES_opencl_crypt(const unsigned char *in, unsigned char *out, size_t nbytes, int enc, cl_mem *device_buffer, cl_mem *device_schedule, cl_command_queue queue, cl_kernel device_kernel, cl_context context) {
	...
	if(!aes_sbox) {
		cl_int error;
		CL_ASSIGN(aes_sbox = clCreateBuffer(context, CL_MEM_READ_ONLY, 4096, NULL, &error));
		CL_WRAPPER(clEnqueueWriteBuffer(queue,aes_sbox,CL_TRUE,0,4096,Te,0,NULL,NULL));
	...
	clEnqueueWriteBuffer(queue,*device_buffer,CL_TRUE,0,nbytes,in,0,NULL,NULL);
		...
		clEnqueueNDRangeKernel(queue,device_kernel, 2, NULL,gridSize, blockSize, 0, NULL, &event);
		...
		fprintf(stdout, "AES      OpenCi %zu bytes, %06lu usecs, %lu Mb/s\n", nbytes, opencl_time, (1000000/opencl_time * 8 * ((unsigned int)nbytes/1024)/1024));
		fprintf(stdout, "AES      OpenCL %zu bytes, %06d usecs, %u Mb/s\n", nbytes, (int)difference.tv_usec, (1000000/(unsigned int)difference.tv_usec * 8 * ((unsigned int)nbytes/1024)/1024));
	#else
		clEnqueueNDRangeKernel(queue,device_kernel, 2, NULL,gridSize, blockSize, 0, NULL, NULL);
	#endif
